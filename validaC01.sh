...
for i in $(ls -d */); do
	...
	if [ -f $Folder/*.c ]; then
		...
		if [ -z $Norme ]; then
			...
			if ! [ -f $BASEDIR/C01/$Folder/main.c ]; then
				printf "\n${Red}Arquivo main não encontrado.\n$BASEDIR/C01/$Folder/main.c${NC}\n\n"
				exit 1
			fi
			GCC=$(gcc -Wall -Wextra -Werror $BASEDIR/C01/$Folder/main.c $Folder/*.c -o $Folder/a.out 2>&1 | head -c1)
			if [ -z $GCC ]; then
				printf "${Green}OK${NC}\n${Yellow}OUTPUT:${NC}\n"
				$Folder/a.out
				rm -fr $Folder/a.out
